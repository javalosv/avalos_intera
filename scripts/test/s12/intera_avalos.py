...
def path(_initial,_goal,_step,_end=True):

	j0=np.linspace(_initial[0], _goal[0], num=_step,endpoint=_end)
	j1=np.linspace(_initial[1], _goal[1], num=_step,endpoint=_end)
	j2=np.linspace(_initial[2], _goal[2], num=_step,endpoint=_end)
	j3=np.linspace(_initial[3], _goal[3], num=_step,endpoint=_end)
	j4=np.linspace(_initial[4], _goal[4], num=_step,endpoint=_end)
	j5=np.linspace(_initial[5], _goal[5], num=_step,endpoint=_end)
	j6=np.linspace(_initial[6], _goal[6], num=_step,endpoint=_end)
	q=np.array([j0,j1,j2,j3,j4,j5,j6])
	return q

def path_full(_path,_steps):
	n=len(_path)
	if (n==2):
		q=path(_path[0],_path[1],_steps[0],True)
	else:
		q=path(_path[0],_path[1],_steps[0],False)
		if(n>3):
			for i in range(n-3):
				q1=path(_path[i+1],_path[i+2],_steps[i+1],False)
				q=np.column_stack((q,q1))
		q2=path(_path[-2],_path[-1],_steps[-1],True)
		q=np.column_stack((q,q2))
	return q
...
def min_time(_q):
	...
	v_factor=0.85 # Es un concepto de seguridad para las pruebas.
	N=len(vel_lim)
	k=len(_q[0])
...
class Opt_avalos():
	def __init__(self,_q,_f,_alfa):
		self.q=_q
		self.f=_f/13 # For optimization we reduce the time with less frecuency
		self.alfa=_alfa
